...
public class BoardDao {
	...
	public ArrayList<BoardVo> getBoardList() {
		...
		try {
			...
			query.append("select no, title, content, user_id, hit, regdate, reply_group, reply_order, reply_indent ");
			query.append("from( ");
			query.append(
					"     select rownum rnum, no, title, content, user_id, hit, regdate, reply_group, reply_order, reply_indent");
			query.append("     from (");
			query.append("		select no, title, content, user_id, hit, regdate, reply_group, reply_order, reply_indent");
			query.append("		from tbl_review");
	...
	public int deleteBoard(int no) {
		...
		try {
			...
			String query = "delete from tbl_review where no=?";
	...
	public int modifyBoard(BoardVo boardVo) {
		...
		try {
			...
			String query = "update tbl_review set title=?, content=?";
			query += " where no=?";
	...
	public int getSeq() {
		...
		try {
			...
			StringBuffer query = new StringBuffer();
			query.append("select SEQ_AFTER_NO.nextval from dual");
	...
	/**
	 * 새 게시물 저장(답글이 아닌) 메소드 - 특이한 점은 sequence를 사용하지 않고 getSeq()라는 메소드를 이용. - 기존 방식대로
	 * 그냥 시퀀스를 사용해도 됨.
	 * 
	 * @param board
	 * @return
	...
	public int insertNewBoard(BoardVo board) {
		...
		try {
			...
			StringBuffer query = new StringBuffer();
			query.append("insert into tbl_review(");
			query.append(" no, user_id, title, content, hit, reply_group, reply_order, reply_indent)");
			query.append(" values ");
			query.append(" (?,?,?,?,?,?,?,?)");
			...
			pstmt.setInt(1, no);
			pstmt.setString(2, board.getId()); // 작성자 ID
			pstmt.setString(3, board.getTitle()); // 제목
	...
	public int insertReplyBoard(BoardVo board) {
		...
		try {
			...
			StringBuffer query = new StringBuffer();
			query.append("insert into tbl_review(");
			query.append("no, user_id, title, content, reply_group, reply_order, reply_indent)");
			query.append(" values ");
			query.append(" (SEQ_AFTER_NO.nextval,?,?,?,?,?,?)");
			System.out.println("SQL :  " + query.toString());
	...
	public void updateHitCount(int no) {
		try {
			...
			String query = "update tbl_review set hit = hit + 1";
			query += " where no = ?";
			System.out.println("SQL :  " + query);
	...
	/**
	 * [답글 저장전 사전작업] commit & rollback 나눠서 처리함! 답글 저장 전에 현재 원글의 order 보다 큰 글들 즉, 기존
	 * 답글들의 order+1 해서 순번이 뒤로 밀리게 함 그리고 밀린 자리에 현재 답글이 위치하게 됨.(답글중 가장 작은 order를 갖게 되어
	 * 부모 바로 밑으로 오게됨)
	 */
	public boolean reqUpdate(int reply_group, int reply_order) {
		...
		try {
			...
			query.append("update tbl_review set reply_order = reply_order + 1");
